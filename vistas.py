...
def spanish_handle_index(environ, start_response, productos=None): # pongo el none para que no sea un parámetro por defecto, es por esto que daba un error, ya que estabamos invocando a este método en controlador.py sin ese parámetro
...
def eventos_anteriores(environ, start_response, eventos):
    ...
    response_headers = [('Content-type', 'text/html')]
    start_response(status, response_headers)
    return [b'Visualizacion de datos']
"""
